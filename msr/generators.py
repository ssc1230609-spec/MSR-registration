...
def volgen_crossmodality(
    ...
    while True:
        ...
        #     print(f"图像 {i}: 最小值 = {np.min(img):.6f}, 最大值 = {np.max(img):.6f}")
        ...
        src_seg = [py.utils.load_volfile(seg_name, **load_params)
                   for seg_name in seg_names[:actual_batch_size]]
        src_segs = split_seg_global(np.concatenate(src_seg, axis=0), labels=labels, downsize=downsize)#one-hot编码
