...
def minmax_norm(x, axis=None):
    ...
    # Ensure output is float type to avoid casting errors with integer input
    return np.divide(x - x_min, x_max - x_min, out=np.zeros_like(x - x_min, dtype=np.float64), where=x_max != x_min)
