...
class Unet(nn.Module):
    ...
    def __init__(self,
                 ...
                 nb_features=None,
                 nb_levels=None,
                 max_pool=2,#最大池化因子，定义每层下采样的比例
                 feat_mult=1,
                 nb_conv_per_level=1,#每层的卷积数
                 half_res=False,
                 norm=False,#是否在卷积块中进行归一化
                 conv_1_1=False):#输入输出特征数不同时，使用1*1卷积进行维度匹配
        """
        Parameters:
        ...
        nb_dec_convs = len(enc_nf)#编码器的总卷积层数
        final_convs = dec_nf[nb_dec_convs:]#提取解码器剩余部分，进行额外卷积层
        dec_nf = dec_nf[:nb_dec_convs]#编码器和解码器对应
        self.nb_levels = int(nb_dec_convs / nb_conv_per_level) + 1#每层的卷积层数，+1是底层的瓶颈层

        if isinstance(max_pool, int):
            max_pool = [max_pool] * self.nb_levels

        # cache downsampling / upsampling operations
        MaxPooling = getattr(nn, 'MaxPool%dd' % ndims)#MaxPool3d，3D池化操作
        self.pooling = [MaxPooling(s) for s in max_pool]#为每个层级创建池化操作，s 是池化核大小（如2表示2×2×2池化）
        self.upsampling = [nn.Upsample(scale_factor=s, mode='nearest') for s in max_pool]

        # configure encoder (down-sampling path)
        prev_nf = infeats#记录上一层的特征通道数，初始值为输入特征数infeats
        encoder_nfs = [prev_nf]#记录每一层编码器的输出通道数，用于跳跃连接
        ...
        for level in range(self.nb_levels - 1):
            ...
            for conv in range(nb_conv_per_level):
                ...
                if prev_nf != nf and conv_1_1:
                    ...
                    convs.append(ConvBlock(ndims, prev_nf, nf, norm=norm))#prev_nf是inchanell，nf是输出通道
                prev_nf = nf
            self.encoder.append(convs)
            encoder_nfs.append(prev_nf)

        # configure decoder (up-sampling path)
        encoder_nfs = np.flip(encoder_nfs)#倒转编码器的特征列表与encoder对齐
...
class VxmFeat(LoadableModel):
    ...
    def __init__(self,
        ...
        # 局部刚性配准支持
        self.use_local_rigid = use_local_rigid
        ...
        if self.use_local_rigid:
            ...
            self.local_rigid_net = LocalRigidNet(
                img_size=inshape,
                feature_channels=nb_feat_extractor[-1][-1],  # 特征提取器的输出通道数
                num_vertebrae=num_vertebrae
            )

    def forward(self, source, target, source_mask=None, return_pos_flow=True,
        ...
        if self.use_local_rigid and source_mask is not None:
            ...
            y_source = self.transformer(source, combined_flow)
        else:
