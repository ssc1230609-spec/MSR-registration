...
# mambad的包不好导入，试试包导入和直接导入两种方式
try:
    from . import configs_TransMorph as configs
...
try:
    ...
    try:
        ...
        print(f"Failed to import MambaLayer: {e}")
try:
    from .LocalRigidNet import LocalRigidNet
except ImportError:
    try:
        from LocalRigidNet import LocalRigidNet
    except ModuleNotFoundError as e:
        print(f"Failed to import LocalRigidNet: {e}")
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x, H, W, T):
        ...
        # Removed: padding logic below handles odd dimensions
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
        patch_size = to_3tuple(patch_size)#转化为3元组，支持3D数据
        self.patch_size = patch_size

        self.in_chans = in_chans
        self.embed_dim = embed_dim

        self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)#使用3D卷积实现图像块划分和线性投影
...
class MambaBlock(nn.Module):
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        self.frozen_stages = frozen_stages
        # split image into non-overlapping patches
        self.patch_embed = PatchEmbed(#patchembed是怎么拆的
            patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
            norm_layer=norm_layer if self.patch_norm else None)
        ...
        for i_layer in out_indices:
            layer = norm_layer(num_features[i_layer])
            layer_name = f'norm{i_layer}'
            self.add_module(layer_name, layer)#794-801有点不太懂要做什么
    ...
    def forward(self, x, skip=None):
        ...
        if skip is not None:
            ...
            if x.shape[2:] != skip.shape[2:]:
                x = torch.nn.functional.interpolate(x, size=skip.shape[2:], mode='trilinear', align_corners=True)
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, src, flow):
        ...
        # check if grid size matches flow size, if not, create a new grid
        if self.grid.shape[2:] != shape:
...
class TransMorph(nn.Module):
    def __init__(self, config):
        ...
        self.integrate = layers.VecInt(config.img_size, config.int_steps if hasattr(config, 'int_steps') else 7)
        
        # 局部刚性配准支持
        self.use_local_rigid = getattr(config, 'use_local_rigid', False)
        if self.use_local_rigid:
            from LocalRigidNet import LocalRigidNet
            num_vertebrae = getattr(config, 'num_vertebrae', 4)
            # reg_head_chan是最后解码器的输出通道数，分成两半作为source和target特征
            self.local_rigid_net = LocalRigidNet(
                img_size=config.img_size,
                feature_channels=config.reg_head_chan // 2,  # 使用实际通道数的一半
                num_vertebrae=num_vertebrae
            )

    def forward(self, source, target, source_mask=None, return_pos_flow=False):
        ...
        # 局部刚性配准
        if self.use_local_rigid and source_mask is not None:
            # 使用解码后的特征x作为特征输入（包含source和target的混合特征）
            # 为了让LocalRigidNet工作，我们将x分成两部分作为source和target特征
            feature_channels = x.size(1) // 2
            source_feat = x[:, :feature_channels, :, :, :]
            target_feat = x[:, feature_channels:, :, :, :]
            rigid_flow = self.local_rigid_net(source_feat, target_feat, source_mask)
            combined_flow = pos_flow + rigid_flow
            moved = self.spatial_trans(source, combined_flow)
        else:
            combined_flow = pos_flow
            moved = self.spatial_trans(source, pos_flow)
        
        ret = {'moved_vol': moved, 'preint_flow': flow}
        if return_pos_flow:
            ret['pos_flow'] = combined_flow
        if self.use_local_rigid and source_mask is not None:
            ret['rigid_flow'] = rigid_flow
            ret['deform_flow'] = pos_flow
        return ret
...
class MambaMorph(nn.Module):
    def __init__(self, config):
        ...
        self.up3 = DecoderBlock(embed_dim, embed_dim // 2, skip_channels=embed_dim // 2 if if_convskip else 0,
                                use_batchnorm=False)  # 384, 80, 80, 128
        self.up4 = DecoderBlock(embed_dim // 2, config.reg_head_chan,#reg_head_cha是什么
                                skip_channels=config.reg_head_chan if if_convskip else 0,
                                use_batchnorm=False)  # 384, 160, 160, 256
        self.c1 = Conv3dReLU(2, embed_dim // 2, 3, 1, use_batchnorm=False)
        self.c2 = Conv3dReLU(2, config.reg_head_chan, 3, 1, use_batchnorm=False)
...
class TransMorphFeat(nn.Module):
    def __init__(self, config):
        ...
        self.feature_extractor = Unet(config.img_size,
                                      ...
                                      half_res=False)
        self.transformer = SwinTransformer(patch_size=config.patch_size,
                                           in_chans=self.feature_extractor.final_nf * 2,
                                           embed_dim=config.embed_dim,
                                           depths=config.depths,
                                           num_heads=config.num_heads,
                                           window_size=config.window_size,
        ...
        self.up4 = DecoderBlock(embed_dim // 2, config.reg_head_chan,
                                skip_channels=config.reg_head_chan if if_convskip else 0,
                                use_batchnorm=False)  # 384, 160, 160, 256
        self.c1 = Conv3dReLU(self.feature_extractor.final_nf * 2, embed_dim // 2, 3, 1, use_batchnorm=False)
        self.c2 = Conv3dReLU(self.feature_extractor.final_nf * 2, config.reg_head_chan, 3, 1, use_batchnorm=False)
        self.reg_head = RegistrationHead(
            in_channels=config.reg_head_chan,
            out_channels=3,
            kernel_size=3,
        ...
        # 局部刚性配准支持
        self.use_local_rigid = getattr(config, 'use_local_rigid', False)
        self.nb_feat_extractor = nb_feat_extractor
        if self.use_local_rigid:
            from LocalRigidNet import LocalRigidNet
            num_vertebrae = getattr(config, 'num_vertebrae', 4)
            self.local_rigid_net = LocalRigidNet(
                img_size=config.img_size,
                feature_channels=self.feature_extractor.final_nf,
                num_vertebrae=num_vertebrae
            )
...
class MambaMorphFeat(nn.Module):
    def __init__(self, config):
        ...
        if_convskip = config.if_convskip
        self.if_convskip = if_convskip#卷积块跳跃连接
        if_transskip = config.if_transskip
        self.if_transskip = if_transskip#Transformer块跳跃连接
        embed_dim = config.embed_dim#Transformer嵌入维度
        nb_feat_extractor = [[16] * 1, [16] * 1]#两个编码器，四个解码器
        self.feature_extractor = Unet(config.img_size,
                                      infeats=1,#输入的特征通道数，CT和MRI分别用一个特征提取器，所以这里的特征通道数是1
                                      nb_features=nb_feat_extractor,
                                      nb_levels=None,#由nb_features自己决定网络层级数
                                      feat_mult=1,#特征倍增因子，1表示每层的特征图数量不变
                                      nb_conv_per_level=1,#每层只进行一次卷积
                                      half_res=False)#是否使用半分辨率
        
        self.use_local_rigid = getattr(config, 'use_local_rigid', False)
        if self.use_local_rigid:
            self.num_vertebrae = getattr(config, 'num_vertebrae', 5)
            self.rigid_flow_weight = getattr(config, 'rigid_flow_weight', 1.0)
            self.deform_flow_weight = getattr(config, 'deform_flow_weight', 1.0)
            self.local_rigid_net = LocalRigidNet(
                feature_channels=self.feature_extractor.final_nf,
        ...
        self.read_out_head = nn.ModuleList()#特征对比学习

        self.read_out_head.append(ConvBlock(len(config.img_size), self.feature_extractor.final_nf,
                                            self.feature_extractor.final_nf // 2, stride=2))
        self.read_out_head.append(ConvBlock(len(config.img_size), self.feature_extractor.final_nf // 2, 3))
    ...
    def forward(self, source, target, source_mask=None, return_pos_flow=True, return_feature=False):
        source_feat = self.feature_extractor(source)
        target_feat = self.feature_extractor(target)
        ...
        x = self.up4(x, f5)
        flow = self.reg_head(x)
        pos_flow = self.integrate(flow)#平滑形变场
        
        if self.use_local_rigid and source_mask is not None:
            rigid_flow = self.local_rigid_net(source_feat, target_feat, source_mask)
            combined_flow = self.deform_flow_weight * pos_flow + self.rigid_flow_weight * rigid_flow
            moved = self.spatial_trans(source, combined_flow)
        else:
            combined_flow = pos_flow
            moved = self.spatial_trans(source, pos_flow)
...
class MambaSwinFusionLayer(nn.Module):
    """
    Fusion layer that combines Mamba and SwinTransformer blocks.
    First processes with Mamba for efficient long-range dependencies,
    then enhances with SwinTransformer block for local attention.
    """
    def __init__(self, dim, depth=2, num_heads=4, window_size=(5, 6, 7),
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, rpe=True,
                 drop=0., attn_drop=0., drop_path=0.,
                 norm_layer=nn.LayerNorm, downsample=None,
                 d_state=16, d_conv=4, expand=2, pat_merg_rf=4,
                 use_checkpoint=False):
        super().__init__()
        self.dim = dim
        self.depth = depth
        ...
        # Mamba component
        self.mamba_norm = norm_layer(dim)
        self.mamba = Mamba(
            d_model=dim,
            d_state=d_state,
            d_conv=d_conv,
        ...
        # Fusion gate to balance Mamba and Swin features
    ...
    def forward(self, x, H, W, T):
        ...
        # Mamba branch
        x_mamba_norm = self.mamba_norm(x)
        if x_mamba_norm.dtype == torch.float16:
            x_mamba_norm = x_mamba_norm.type(torch.float32)
        x_mamba = self.mamba(x_mamba_norm)
        x_mamba = x_mamba.type(x.dtype)
        
        # Swin branch - calculate attention mask
        Hp = int(np.ceil(H / self.window_size[0])) * self.window_size[0]
        Wp = int(np.ceil(W / self.window_size[1])) * self.window_size[1]
        Tp = int(np.ceil(T / self.window_size[2])) * self.window_size[2]
        ...
        for blk in self.swin_blocks:
            blk.H, blk.W, blk.T = H, W, T
            if self.use_checkpoint:
                x_swin = checkpoint.checkpoint(blk, x_swin, attn_mask)
            else:
                x_swin = blk(x_swin, attn_mask)
        
        # Fusion: combine Mamba and Swin features
        x_cat = torch.cat([x_mamba, x_swin], dim=-1)
        gate = self.fusion_gate(x_cat)
        x_fused = gate * x_mamba + (1 - gate) * x_swin
        
        # Residual connection
        x_out = x + x_fused
        
        if self.downsample is not None:
            x_down = self.downsample(x_out, H, W, T)
            Wh, Ww, Wt = (H + 1) // 2, (W + 1) // 2, (T + 1) // 2
            return x_out, H, W, T, x_down, Wh, Ww, Wt
        else:
            return x_out, H, W, T, x_out, H, W, T


class MambaSwinFusionBlock(nn.Module):
    """
    Backbone that uses MambaSwinFusionLayer for each stage.
    Combines the efficiency of Mamba with the local attention of SwinTransformer.
    """
    def __init__(self, pretrain_img_size=224,
                 patch_size=4,
                 in_chans=3,
    ...
    def train(self, mode=True):
        super(MambaSwinFusionBlock, self).train(mode)
        self._freeze_stages()



class MambaMorphSwinFeat(nn.Module):
    """
    MambaMorphSwinFeat: MambaMorphSwin with feature extractor.
    Combines Mamba's efficient sequence modeling with SwinTransformer's 
    local attention, plus a UNet feature extractor for better feature learning.
    """
    def __init__(self, config):
        super(MambaMorphSwinFeat, self).__init__()
        if_convskip = config.if_convskip
        self.if_convskip = if_convskip
        if_transskip = config.if_transskip
        self.if_transskip = if_transskip
        ...
        # ReadOut head for feature contrastive learning
        self.read_out_head = nn.ModuleList()
        self.read_out_head.append(ConvBlock(len(config.img_size), self.feature_extractor.final_nf,
                                            self.feature_extractor.final_nf // 2, stride=2))
        self.read_out_head.append(ConvBlock(len(config.img_size), self.feature_extractor.final_nf // 2, 3))
        
        self.transformer = MambaSwinFusionBlock(
    ...
    def forward(self, source, target, source_mask=None, return_pos_flow=True, return_feature=False):
        ...
        if return_feature:
            ...
            tgt_feat = target_feat
            for head in self.read_out_head:
                src_feat = head(src_feat)
                tgt_feat = head(tgt_feat)
            ret['feature'] = torch.cat((src_feat, tgt_feat), dim=1)
        return ret
...
CONFIGS = {
    ...
    'MambaMorphSwin': configs.get_3DMambaMorphSwin_config(),
}
